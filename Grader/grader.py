...
def target_encoding(name, dataframe, m=1):
    ...
    dataframe = dataframe.explode(name)
    overall = dataframe["label"].mean()
    dataframe = dataframe.groupby(name).agg(
        freq=("label", "count"),
        in_category=("label", np.mean)
    ).reset_index()
    dataframe["weight"] = dataframe["freq"] / (dataframe["freq"] + m)
    dataframe["score"] = dataframe["weight"] * dataframe["in_category"] + (1 - dataframe["weight"]) * overall
    return dataframe
...
select_cols = ['StudentAbsenceDays', 'Relation', 'VisitedResources']
# 'Topic', 'Semester', 'PlaceofBirth', 'raisedhands', 'StageID',
# 'AnnouncementsView', 'Discussion', 'Nationality', 'SectionID',
# 'ParentschoolSatisfaction', 'gender', 'GradeID', 'ParentAnsweringSurvey']
